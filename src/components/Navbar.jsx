      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden md:flex md:items-center md:gap-8">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.key}
                  to={path}
                  className={`text-sm font-semibold transition ${location.pathname === path ? 'text-primary-700' : 'text-slate-600 hover:text-primary-700'}`}
                >
                  {t(`nav.${link.key}`)}
                </Link>
              )
            ...
            >
              {t(lang === 'ar' ? 'nav.switchToEnglish' : 'nav.switchToArabic')}
            </Link>
            <a
              href={LMS_URL}
              target="_blank"
        ...
        <div className="md:hidden">
          <div className="space-y-1 border-t border-slate-200 bg-white px-4 pb-4 pt-2">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-md px-3 py-2 text-base font-medium ${location.pathname === path ? 'bg-primary-50 text-primary-700' : 'text-slate-700 hover:bg-slate-100'}`}
                >
                  {t(`nav.${link.key}`)}
                </Link>
              )
            ...
            <Link
              to={switchPath}
              onClick={() => setIsOpen(false)}
              className="block rounded-md px-3 py-2 text-base font-medium text-primary-700 hover:bg-primary-50"
            >
              {t(lang === 'ar' ? 'nav.switchToEnglish' : 'nav.switchToArabic')}
            </Link>
            <a
