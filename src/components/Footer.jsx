    ...
    <footer className="border-t border-slate-200 bg-white">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-sm text-slate-600">
              {links.map(([key, path]) => (
                <li key={key}>
                  <Link to={localePath(lang, path)} className="hover:text-primary-700">
                    {t(`nav.${key}`)}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm text-slate-600">
              <li className="flex items-start gap-3">
                ...
                <span>{t('footer.onlineWorldwide')}</span>
              </li>
              <li className="flex items-start gap-3">
              ...
              <li className="flex items-start gap-3">
                ...
                <a href={`mailto:${t('footer.email')}`} className="hover:text-primary-700">
                  {t('footer.email')}
                </a>
              </li>
            </ul>
        ...
        <div className="mt-12 border-t border-slate-200 pt-8 text-center text-sm text-slate-500">
          <p>{t('footer.designedBy', { year: new Date().getFullYear() })}</p>
        </div>
      </div>
    </footer>
  )
